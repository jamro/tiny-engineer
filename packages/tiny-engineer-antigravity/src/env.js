...
/**
 * Parse a .env file and set process.env for keys not already set.
...
function loadFile(filePath) {
  try {
    ...
    // Missing file / read errors ignored.
  }
}

/**
 * Load .env from cwd and ~/.gemini/config/.env.
 * @param {string} [cwd]
 */
export function loadDotEnv(cwd = process.cwd()) {
  loadFile(join(cwd, ".env"));
  loadFile(join(homedir(), ".gemini", "config", ".env"));
}
