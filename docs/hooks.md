...
Project hooks drive Tiny Engineer poses from Cursor agent events. Config lives in [`.cursor/hooks.json`](../.cursor/hooks.json). The animation CLI is [`packages/tiny-engineer/`](../packages/tiny-engineer/).
...
4. Use **Node.js 18+** (hooks call `node packages/tiny-engineer/bin/tiny-engineer.js`).

5. Optional smoke test (robot should ring):

```bash
echo '{"hook_event_name":"stop"}' | node packages/tiny-engineer/bin/tiny-engineer.js
node packages/tiny-engineer/bin/tiny-engineer.js --help
```
...
Each anim hook runs the same command with **no animation args**. Cursor pipes event JSON on stdin; the CLI reads `hook_event_name` (and `tool_name` for `preToolUse`), maps to a pose in [`packages/tiny-engineer/src/map.js`](../packages/tiny-engineer/src/map.js), and `POST`s `/anim?name=…` (2s timeout, exit 0). Optional `--url` overrides the default `http://tiny-engineer.local`.
...
- To change the map, edit [`packages/tiny-engineer/src/map.js`](../packages/tiny-engineer/src/map.js).
